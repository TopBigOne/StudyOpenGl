...
public class BlendingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myGLSurfaceView = findViewById(R.id.mgl_blending_render);
        glRender = new GLRender();
        glRender.init();

        glRender.setParamsInt(SAMPLE_TYPE, SAMPLE_TYPE_BLENDING, 0);
        myGLSurfaceView.setNativeRender(glRender.getNativeRender());

        loadRGBAImage(R.mipmap.board_texture, 0);
        loadRGBAImage(R.mipmap.floor, 1);
        loadRGBAImage(R.mipmap.window, 2);
