...
void EGLRender::Init() {
    ...
    // 获取shader 中uniform 变量句柄
    m_SamplerLoc = glGetUniformLocation(m_ProgramObj, "s_TextureMap");
    m_TexSizeLoc = glGetUniformLocation(m_ProgramObj, "u_texSize");

    // 创建VBO和VEO
    glGenBuffers(3, m_VboIds); //  index 2 : 表示的是VEO
    // 设置VBO 和 VEO buffer 数据
    //  顶点
...
int EGLRender::CreateGlesEnv() {
    ...
    // surface attributes
    // the surface size is set to input frame size;
    ...
    int resultCode = 0;
    do {
        // 1. 获取EGLDisplay 对象，建立与本地窗口系统的连接
        ...
        // 4. c创建渲染表面EGLSurface ,使用eglCreatePbufferSurface 创建屏幕外渲染区域
...
void EGLRender::SetImageData(uint8_t *pData, int width, int height) {
    LOGCATI("EGLRender::SetImageData pData = %p, [w,h] = [%d, %d]", pData, width, height);
    if (pData && m_IsGLContextReady) {
        ...
        glBindTexture(GL_TEXTURE_2D, m_ImageTextureId);
        // - 根据指定的参数，生成一个2D纹理（Texture）,- 即：将载入的位图文件(*.bmp)转换成纹理贴图。
        ...
        if (m_FboId == GL_NONE) {
            ...
            glBindTexture(GL_TEXTURE_2D, m_FboTextureId);
            // 将2维纹理对象绑定到 帧缓冲区，帧缓冲区本身是不存放颜色、深度等信息的，
...
void EGLRender::Draw() {
    ...
    if (m_TexSizeLoc > -1) {
        ...
        // 给 uniform 类型的变量 m_TexSizeLoc 赋值
