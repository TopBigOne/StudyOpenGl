...
void BigEyesSample::Init() {
    ...
    // step 3: create vertex.
    glGenVertexArrays(1, &m_VaoId);
    // step 3-1 : bind vertex
    glBindVertexArray(m_VaoId);

    glBindBuffer(GL_ARRAY_BUFFER, m_VboIds[0]);
    // enable the vertex attribute : layout(location = 0) in vec4 a_position in vertex.
    glEnableVertexAttribArray(0);
    // tell GPU how to parse the vertex array data.
