...
glm::vec3 m_pVtxCoords[TRIANGLE_NUM * 3];
glm::vec2 m_pTexCoords[TRIANGLE_NUM * 2];

glm::vec2 m_CurTouchPoint;
glm::vec2 m_PreTouchPoint;
 ...
 */
glm::vec2 binaryEquationGetKB(float x1, float y1, float x2, float y2) {
    float k = (y1 - y2) / (x1 - x2);
 ...
 * ax² + bx + c = 0
 */
glm::vec2 quadEquationGetX(float a, float b, float c) {
    glm::vec2 xArr;
 ...
 */
glm::vec4
getInsertPointBetweenCircleAndLine(float x1, float y1, float x2, float y2, float m, float n,
                                   float r) {
    glm::vec2 kbArr = binaryEquationGetKB(x1, y1, x2, y2);
    float     k     = kbArr.x;
    float     b     = kbArr.y;
    ...
    glm::vec4 insertPoints;
    glm::vec2 xArr = quadEquationGetX(aX, bX, cX);
...
void ScratchCardSample::Draw(int screenW, int screenH) {
    ...
    // 当片段的模板值不为1时，片段通过测试进行渲染。
    glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
    // 当模板测试和深度测试都通过了，将片段对应的模板值 都替换成1；
    ...
    for (const auto &pre_cur_point: m_PointVector) {
        CalculateMesh(glm::vec2(pre_cur_point.x, pre_cur_point.y),
                      glm::vec2(pre_cur_point.z, pre_cur_point.w));
        glBindBuffer(GL_ARRAY_BUFFER, m_VboIds[0]);
...
void ScratchCardSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    ...
    Model = glm::scale(Model, glm::vec3(m_ScaleX, m_ScaleY, 1.0));
    Model = glm::rotate(Model, radiansX, glm::vec3(1.0, 0.0f, 0.0f));
    Model = glm::rotate(Model, radiansY, glm::vec3(0.0f, 1, 0.0f));
...
void ScratchCardSample::CalculateMesh(glm::vec2 pre, glm::vec2 cur) {
    glm::vec2 imgSize(m_RenderImage.width, m_RenderImage.height);
    glm::vec2 p0 = pre * imgSize, p1 = cur * imgSize;
    ...
    {
        v0 = glm::vec2(p0.x, p0.y - r) / imgSize;
        v1 = glm::vec2(p0.x, p0.y + r) / imgSize;
        v2 = glm::vec2(p1.x, p1.y - r) / imgSize;
        v3 = glm::vec2(p1.x, p1.y + r) / imgSize;

    } else if (p0.x == p1.x) { //2. 平行于 y 轴的
        v0 = glm::vec2(p0.x - r, p0.y) / imgSize;
        v1 = glm::vec2(p0.x + r, p0.y) / imgSize;
        v2 = glm::vec2(p1.x - r, p1.y) / imgSize;
        v3 = glm::vec2(p1.x + r, p1.y) / imgSize;
        ...
        glm::vec4 v0_v1 = getInsertPointBetweenCircleAndLine(x0, y0, x0_i, y0_i, x0, y0, r);

        v0 = glm::vec2(v0_v1.x, v0_v1.y) / imgSize;
        v1 = glm::vec2(v0_v1.z, v0_v1.w) / imgSize;

        glm::vec4 v2_v3 = getInsertPointBetweenCircleAndLine(x1, y1, x1_i, y1_i, x1, y1, r);

        v2 = glm::vec2(v2_v3.x, v2_v3.y) / imgSize;
        v3 = glm::vec2(v2_v3.z, v2_v3.w) / imgSize;
    ...
    for (int i     = 0; i < 20; ++i) {
        ...
        m_pTexCoords[index + 6 * i + 0] = glm::vec2(x, y) / imgSize;
        m_pTexCoords[index + 6 * i + 1] = glm::vec2(x_, y_) / imgSize;
        m_pTexCoords[index + 6 * i + 2] = glm::vec2(x0, y0) / imgSize;
        ...
        m_pTexCoords[index + 6 * i + 3] = glm::vec2(x, y) / imgSize;
        m_pTexCoords[index + 6 * i + 4] = glm::vec2(x_, y_) / imgSize;
        m_pTexCoords[index + 6 * i + 5] = glm::vec2(x1, y1) / imgSize;
    }
...
void ScratchCardSample::SetTouchLocation(float x, float y) {
    ...
    if (m_SurfaceWidth * m_SurfaceHeight != 0) {
        ...
        if (m_bReset) {
            if (x != -1) {
                m_CurTouchPoint = glm::vec2(x / m_SurfaceWidth, y / m_SurfaceHeight);
                m_bReset        = false;
            }
        } else {
