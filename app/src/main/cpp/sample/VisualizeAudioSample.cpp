...
void VisualizeAudioSample::Init() {
    if (m_ProgramObj)
        return;
    LOGCATE("VisualizeAudioSample:: Init");
    char vShaderStr[] =
            "#version 300 es\n"
...
VisualizeAudioSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGCATI("VisualizeAudioSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX,
            angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
...
void VisualizeAudioSample::UpdateMesh() {
    LOGCATI("VisualizeAudioSample:: UpdateMesh");
    //calculate mesh
