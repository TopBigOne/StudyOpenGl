...
void VisualizeAudioSample::Draw(int screenW, int screenH) {
    glClearColor(ONE_FLOAT, ONE_FLOAT, ONE_FLOAT, ONE_FLOAT);
    if (m_ProgramObj == GL_NONE) {
        return;
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(25));
    std::unique_lock<std::mutex> lock(m_Mutex);
    if (!m_bAudioDataReady) {
        return;
    }
    UpdateMesh();
    lock.unlock();
...
void VisualizeAudioSample::Destroy() {
    ...
    GLUtils::setFloat(m_ProgramObj, "drawType", ONE_FLOAT);
    glDrawArrays(GL_TRIANGLES, 0, m_RenderDataSize * 6);


    GLUtils::setFloat(m_ProgramObj, "drawType", ZERO_FLOAT);
    glDrawArrays(GL_TRIANGLES, 0, m_RenderDataSize * 6);
}
...
void VisualizeAudioSample::LoadShortArrData(short *const pShortArr, int arrSize) {
    LOGCATI("VisualizeAudioSample::LoadShortArrData pShortArr=%p, arrSize=%d", pShortArr, arrSize);
    ...
    std::unique_lock<std::mutex> lock(m_Mutex);
    if (m_FrameIndex == 1) {
        m_pAudioBuffer = new short[arrSize >> 1];
    ...
    if (m_FrameIndex == 2) {
        ...
        // 顶点坐标： (x,y,z)* 6 points;
        m_pVerticesCoords = new glm::vec3[m_RenderDataSize * 6];
        // 纹理坐标 (x,y) *6 points;
        m_pTextureCoords  = new glm::vec2[m_RenderDataSize * 6];
    }
    if (m_FrameIndex > 2) {
...
void VisualizeAudioSample::UpdateMesh() {
    ...
    if (m_pAudioBuffer + m_AudioDataSize - m_pCurAudioData >= step) {
        LOGCATE("VisualizeAudioSample::UpdateMesh() m_pAudioBuffer + m_AudioDataSize - m_pCurAudioData >= step");
        ...
        LOGCATI("VisualizeAudioSample::Draw() m_pAudioBuffer + m_AudioDataSize - m_pCurAudioData < step");
        m_bAudioDataReady = false;
        m_Cond.notify_all();
        return;
    }
