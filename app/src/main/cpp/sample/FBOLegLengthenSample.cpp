...
void FBOLegLengthenSample::Init() {
    ...
    if (m_FboProgramObj) {
        ...
        glBindTexture(GL_TEXTURE_2D, m_FboTextureId);
        GLsizei width  = m_RenderImage.width;
        GLsizei height = m_RenderImage.height;
        if (m_bIsVerticalMode) {
            height = static_cast<GLsizei>(height * (1 + 2 * m_dt));

        } else {
            width = static_cast<GLsizei>(width * (1 + 2 * m_dt));
...
void FBOLegLengthenSample::configStretchArea() {
    auto  imgWidth  = static_cast<float>(m_RenderImage.width);
    auto  imgHeight = static_cast<float>(m_RenderImage.height);
    RectF inRectF;

    // 直白点： 高度是图片的一半，其他的长度，保持一致。
    inRectF.left   = 0.0f;
    inRectF.right  = imgWidth;
    inRectF.top    = imgHeight * 0.5f;
    inRectF.bottom = imgHeight;

    // 0 ;
    m_StretchRect.left   = inRectF.left / imgWidth;
    // 1
    m_StretchRect.right  = inRectF.right / imgWidth;
    // 0.5;
    m_StretchRect.top    = inRectF.top / imgHeight;
    // 1
    m_StretchRect.bottom = inRectF.bottom / imgHeight;

}

void FBOLegLengthenSample::claculateStretchMode() {
    // case1：垂直
    if (m_bIsVerticalMode) {
...
void FBOLegLengthenSample::handleStretch() {
...
void FBOLegLengthenSample::initMVP() {
...
void FBOLegLengthenSample::initShaderProgram() {
...
void FBOLegLengthenSample::initShaderData() {
    float y1 = 1 - 2 * m_StretchRect.top;
    float y2 = 1 - 2 * m_StretchRect.bottom;
    float x1 = 2 * m_StretchRect.left - 1;
    float x2 = 2 * m_StretchRect.right - 1;
...
//	};
    ...
    if (m_bIsVerticalMode) {
        ...
        glGenBuffers(6, m_VboIds);
        glBindBuffer(GL_ARRAY_BUFFER, m_VboIds[0]);
        glBufferData(GL_ARRAY_BUFFER, sizeof(vTexCoors), vTexCoors,
                     GL_DYNAMIC_DRAW);
    }
...
void FBOLegLengthenSample::initDT() {
    if (m_dt <= -0.2) {
        m_isgo = true;
    }
...
void FBOLegLengthenSample::Draw(int screenW, int screenH) {
    LOGCATI("FBOLegLengthenSample::Draw [screenW, screenH] = [%d, %d]", screenW, screenH);
    ...
    GO_CHECK_GL_ERROR();
    GLsizei indicesNum = handleIndexNum();
...
GLsizei FBOLegLengthenSample::handleIndexNum() {
    GLsizei indicesNum;
    switch (m_StretchMode) {
        case VERTICAL_STRETCH_8_POINTS:
        case HORIZONTAL_STRETCH_8_POINTS:
