...
void TransformFeedbackSample::Init() {
    ...
    // 3+2: 顶点坐标+纹理坐标
    // 6: todo : 什么意思？
    glBufferData(GL_TRANSFORM_FEEDBACK_BUFFER, (3 + 2) * 6 * sizeof(GLfloat), nullptr,
                 GL_STATIC_READ);
...
void TransformFeedbackSample::Draw(int screenW, int screenH) {
    ...
    // read the feedback buffer
    glBindBuffer(GL_TRANSFORM_FEEDBACK_BUFFER, m_TransFeedbackBufId);
    void     *rawData = glMapBufferRange(GL_TRANSFORM_FEEDBACK_BUFFER, 0,
                                         (3 + 2) * 6 * sizeof(GLfloat),
                                         GL_MAP_READ_BIT);
    auto     *p       = (float *) rawData;
    for (int i        = 0; i < 6; i++) {
