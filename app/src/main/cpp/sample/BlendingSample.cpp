...
BlendingSample::~BlendingSample() {
    for (auto &renderImage: m_RenderImages) {
...
void BlendingSample::Init() {
    ...
    for (const auto &item: m_TextureIds) {
...
void BlendingSample::LoadMultiImageWithIndex(int index, NativeImage *pImage) {
    ...
    LOGCATI("BlendingSample::LoadImage index=%d, pImage = %p", index, pImage->ppPlane[0]);
    if (pImage) {
...
void BlendingSample::Draw(int screenW, int screenH) {
    ...
    glEnable(GL_BLEND);
    // 混合方式 :  源因子 和 目标因子
    // 插值混合能通过调用glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
