...
void FaceSlenderSample::Init() {
    if (m_ProgramObj)
        return;
    effectRadius = PointUtil::Distance(PointF(LeftCheekKeyPoint[0], LeftCheekKeyPoint[1]),
    ...
    char fShaderStr[] =
                 ...
                 "\n"
                 "    vec2 newTexCoord = face_slender_2(leftPreCtrl, leftCurCtrl, v_texCoord, u_reshapeRadius, u_texSize);\n"
...
void FaceSlenderSample::LoadImage(NativeImage *pImage) {
    LOGCATI("FaceSlenderSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
void FaceSlenderSample::Draw(int screenW, int screenH) {
    LOGCATD("FaceSlenderSample::Draw() [w,h]=[%d,%d]", screenW, screenH);
