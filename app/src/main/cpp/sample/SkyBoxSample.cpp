...
void SkyBoxSample::Init() {
    ...
    // generate cube VBO and load the VBOS with data
...
void SkyBoxSample::LoadImage(NativeImage *pImage) {
    LOGCATI("SkyBoxSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
void SkyBoxSample::Draw(int screenW, int screenH) {
    ...
    UpdateMVPMatrix(m_MVPMatrix, m_AngleX, m_AngleY, 1.0f, (float) screenW / screenH);
    if (!m_TextureId) {
        ...
        for (int i = 0; i < sizeof(m_pSkyBoxRenderImg) / sizeof(NativeImage); ++i) {
            ...
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA,
                         m_pSkyBoxRenderImg[i].width, m_pSkyBoxRenderImg[i].height, 0, GL_RGBA,
    ...
    UpdateMVPMatrix(m_MVPMatrix, m_AngleX, m_AngleY, 0.2f, (float) screenW / screenH);
     ...
     *  glm::mat4 View    = glm::lookAt(
            ...
            glm::vec3(0, 1, 1));
     * */
    glUniform3f(m_ViewPosLoc, 0, 0, 1.8f);
    // 激活纹理
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, m_TextureId);
...
void SkyBoxSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float scale,
    ...
    angleX %= 360;
    angleY %= 360;
    auto radiansX = static_cast<float>(MATH_PI / 180 * angleX);
    auto radiansY = static_cast<float>(MATH_PI / 180 * angleY);

    glm::mat4 Project = glm::perspective(45.0f, ratio, 0.1f, 100.0f);
    glm::mat4 View    = glm::lookAt(
            glm::vec3(0, 0, 1.8),
            glm::vec3(0, 0, -1),
            glm::vec3(0, 1, 1));
    ...
    // 哈哈哈哈哈 ，，这个旋转公式，，广亚，应该理解的哈，，after all 你学过啊。
    Model         = glm::rotate(Model, radiansX, glm::vec3(1, 0, 0));
    Model         = glm::rotate(Model, radiansY, glm::vec3(0, 1, 0));
    // mock 平移？
    Model         = glm::translate(Model, glm::vec3(0, 0, 0));
    m_ModelMatrix = Model;
    mvpMatrix     = Project * View * Model;
}
